...
class ClusterCatalogue:
    ...
    def get_cluster_catalogue(self, log10m_true, z_true):
        ...
        if self.settings['cluster_catalogue']['completeness']=='True':
            a=1
        ...
        z_obs = _photometric_redshift.photometric_redshift(z_true, float(self.settings['cluster_catalogue.photometric_redshift']['sigma_z0']))
        z_obs[z_obs < 0] = None

        if self.settings['cluster_catalogue']['purity']=='True': 
        ...
        return richness[mask_selection], log10mWL[mask_selection], z_obs[mask_selection]
